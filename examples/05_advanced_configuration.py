...
def main():
    ...
    memory_efficient_policy = DynamicLocalThresholdPolicy(
        delta=0.03
    )  # 3% max error rate
    vcache_memory_efficient = VCache(
        config=memory_efficient_config, policy=memory_efficient_policy
    )

    demonstrate_configuration(
        vcache_memory_efficient, "Memory-Efficient Setup", test_prompts
    )
    ...
    demonstrate_configuration(
        vcache_research, "Research/Development Setup", test_prompts
    )
