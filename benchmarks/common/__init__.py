...
def get_row_embeddings(tokenizer, model, row, truncation_limit):
    ...
    row_tokens = tokenizer(row, return_tensors='pt', add_special_tokens=False, max_length=truncation_limit, truncation=True).to(device)
    with torch.no_grad():
...
def warm_up_vectorq_server():
    ...
    vectorQ_response = send_request_to_vectorQ(vectorQ_prompt)
    vectorQ_answer = vectorQ_response['results'][0]
