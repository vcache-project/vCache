...
class Benchmark(unittest.TestCase):
        ...
        try:
            with open(self.filepath, 'rb') as file:  # Open in binary mode for ijson
                ...
                for idx, data_entry in enumerate(data_entries):
                    ...
                    emb_generation_latency = float(data_entry[self.embedding_model[0] + '_lat'])
                    llm_generation_latency = float(data_entry[self.llm_model[0] + '_lat'])
                    
                    start_time_direct = time.time()
                    direct_answer = data_entry[self.llm_model[0]]
                    duration_direct = time.time() - start_time_direct + llm_generation_latency
                    ...
                    if cache_hit:
                        ...
                        if answers_have_same_meaning_static(self.task, direct_answer, vectorQ_answer):
                            self.false_negative_counter += 1  # Should have reused but didn't
                        else:
    ...
    def append_results(self, idx, duration_direct, duration_vectorq, answer_reused, direct_answer):
        error_rate_relative = (self.incorrect_answers / self.total_reused * 100) if self.total_reused > 0 else 0
        ...
        relative_reuse_rate = (reused_this_interval / self.step_size) * 100 if self.step_size > 0 else 0
        relative_error_rate_to_step_size = (self.incorrect_answers_in_step_size / reused_this_interval) * 100 if reused_this_interval > 0 else 0
        ...
        precision = self.true_positive_counter / (self.true_positive_counter + self.false_positive_counter) if (self.true_positive_counter + self.false_positive_counter) > 0 else 0
        recall = self.true_positive_counter / (self.true_positive_counter + self.false_negative_counter) if (self.true_positive_counter + self.false_negative_counter) > 0 else 0
        accuracy = (self.true_positive_counter + self.true_negative_counter) / (self.true_positive_counter + self.false_positive_counter + self.true_negative_counter + self.false_negative_counter) if (self.true_positive_counter + self.false_positive_counter + self.true_negative_counter + self.false_negative_counter) > 0 else 0
        ...
        if not answer_reused:
            ...
            entry_size_mb = calculate_cache_entry_size_mb(embedding_dim, embedding_precision, direct_answer)
            self.cache_size_mb += entry_size_mb
        self.cache_size_list.append(self.cache_size_mb)

        if (idx == 0):
            logging.info(f"VectorQ Config (Client) | Task: {self.task}, Output Format: {self.output_format}, Embedding Model: {self.embedding_model[1]}, LLM Model: {self.llm_model[1]}, Is Dynamic Threshold: {self.is_dynamic_threshold}, Threshold: {self.threshold}, Rnd_num_ub: {self.rnd_num_ub}")
    ...
    async def get_vectorQ_answer(self, review_text, row_embedding):
        try:
            ...
            logging.error(f"Could not find embedding in the dataset for {self.embedding_model[0]}")
            raise e
        
        try:
            candidate_response = self.current_data_entry[self.llm_model[0]]
        except json.JSONDecodeError as e:
            logging.error(f"Could not find LLM response in the dataset for {self.llm_model[0]}")
            raise e
        ...
        if isinstance(row_embedding, list):
            row_embedding = [float(val) if hasattr(val, '__float__') else val for val in row_embedding]
        
        vectorQ_benchmark = VectorQBenchmark(
            candidate_embedding=row_embedding,
            candidate_response=candidate_response
        )

        vectorQ_prompt = f"{self.task} {review_text}"
        try:
            vectorQ_response, cache_hit = await vectorq.create(prompt=vectorQ_prompt, output_format=self.output_format, benchmark=vectorQ_benchmark)
        except Exception as e:
...
def calculate_cache_entry_size_mb(embedding_dim, embedding_precision, cached_response):
    bytes_per_float = 2 if embedding_precision == "float16" else 4  # float16 = 2 bytes, float32 = 4 bytes
    ...
    total_size_mb = (embedding_size_bytes + response_size_bytes + metadata_size_bytes) / (1024 * 1024)
    return total_size_mb

def compare_static_vs_dynamic(dataset, embedding_model_name, llm_model_name, timestamp, results_dir=None):
    ...
    plot_hit_rate_vs_error(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    plot_precision_vs_recall(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    plot_duration_comparison(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    plot_duration_vs_error_rate(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
...
if __name__ == '__main__':    
    ...
    for dataset in datasets:
        ...
        for embedding_model in embedding_models:
            logging.info(f"Running benchmark for dataset: {dataset}, embedding model: {embedding_model[1]}\n\n")
            start_time_embedding_model = time.time()
            
            for llm_model in llm_models:
                logging.info(f"Running benchmark for dataset: {dataset}, embedding model: {embedding_model[1]}, LLM model: {llm_model[1]}\n")
                ...
                if THRESHOLD_TYPE in ['static', 'both']:
                    for threshold in static_thresholds:
                        ...
                        benchmark.output_folder_path = os.path.join(results_dir, dataset, embedding_model[1], llm_model[1], f"static_{threshold}")
                ...
                logging.info(f"LLM Model Time: {(end_time_llm_model - start_time_llm_model) / 60:.2f} minutes, {(end_time_llm_model - start_time_llm_model) / 3600:.4f} hours")
            end_time_embedding_model = time.time()
            logging.info(f"Embedding Model Time: {(end_time_embedding_model - start_time_embedding_model) / 60:.2f} minutes, {(end_time_embedding_model - start_time_embedding_model) / 3600:.4f} hours")
