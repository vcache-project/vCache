...
def _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir):
    """
...
def plot_hit_rate_vs_error(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    
    if not static_files and not dynamic_files:
        print(f"No results found for {dataset}, {embedding_model_name}, {llm_model_name}")
        return
    ...
    # Dictionary to group vectorq results by rnd_num_ub
    vectorq_results = {}  # {rnd_num_ub: {'hit_rates': [], 'error_rates': []}}
    ...
    for file_path in static_files:
        with open(file_path, 'r') as f:
            ...
            error_rate = data["error_rates_relative_to_reused_answers"][-1]
    ...
    # Find all vectorq files, and get the data by rnd_num_ub
    for file_path in dynamic_files:
        with open(file_path, 'r') as f:
            ...
            hit_rate = (total_reused / total_samples) * 100 if total_samples > 0 else 0
            error_rate = data["error_rates_relative_to_reused_answers"][-1]
            rnd_num_ub = data["config"]["rnd_num_ub"]
            
            # Check if this is a vectorq_X directory
            if "vectorq_" in file_path:
                if rnd_num_ub not in vectorq_results:
                    vectorq_results[rnd_num_ub] = {'hit_rates': [], 'error_rates': []}
                
                vectorq_results[rnd_num_ub]['hit_rates'].append(hit_rate)
                vectorq_results[rnd_num_ub]['error_rates'].append(error_rate)
            else:
                # Legacy support for old files
                dynamic_hit_rates.append(hit_rate)
                dynamic_error_rates.append(error_rate)
                dynamic_rnd_ubs.append(rnd_num_ub)
    
    # For new runs with multiple files per rnd_num_ub, compute average and confidence intervals
    if vectorq_results:
        # For each rnd_num_ub, compute average and append to dynamic lists
        for rnd_num_ub, values in sorted(vectorq_results.items(), key=lambda x: float(x[0])):
    ...
    if static_hit_rates:
        static_points = sorted(zip(static_hit_rates, static_error_rates, static_thresholds), key=lambda x: x[0])
        ...
        plt.plot(static_hit_rates_sorted, static_error_rates_sorted, 'b-', alpha=0.5, linewidth=3)
        plt.scatter(static_hit_rates_sorted, static_error_rates_sorted, c='blue', marker='o', s=140, label='State of the Art')
    ...
    if dynamic_hit_rates:
        dynamic_points = sorted(zip(dynamic_hit_rates, dynamic_error_rates, dynamic_rnd_ubs), key=lambda x: x[0])
        ...
        plt.plot(dynamic_hit_rates_sorted, dynamic_error_rates_sorted, 'r-', alpha=0.7, linewidth=3)
        plt.scatter(dynamic_hit_rates_sorted, dynamic_error_rates_sorted, c='red', marker='^', s=140, label='VectorQ')
        ...
        if vectorq_results:
            ...
            for i, rnd_ub in enumerate(dynamic_rnd_ubs_sorted):
                if rnd_ub in vectorq_results and len(vectorq_results[rnd_ub]['hit_rates']) > 1:
                    ...
                    error_rate_ci = t_factor * (error_rate_std / np.sqrt(len(error_rates)))
    ...
    plt.legend(fontsize=FONT_SIZE, loc='upper center', bbox_to_anchor=(0.5, 1.167), 
              ncol=3, frameon=True, fancybox=True, columnspacing=1.0, handletextpad=0.4)

    output_dir = f"processed_{results_dir}{dataset}/{embedding_model_name}/{llm_model_name}/"
...
def plot_precision_vs_recall(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    ...
    # Dictionary to group vectorq results by rnd_num_ub
    vectorq_results = {}  # {rnd_num_ub: {'precisions': [], 'recalls': []}}
    ...
    for file_path in dynamic_files:
        with open(file_path, 'r') as f:
            ...
            if "vectorq_" in file_path:
                ...
                vectorq_results[rnd_num_ub]['precisions'].append(precision)
                vectorq_results[rnd_num_ub]['recalls'].append(recall)
            else:
                # Legacy support for old files
    ...
    # Plot static threshold points
    if static_recalls:
        static_points = sorted(zip(static_recalls, static_precisions, static_thresholds), key=lambda x: x[0])
        static_recalls_sorted = [point[0] for point in static_points]
        static_precisions_sorted = [point[1] for point in static_points]
        static_thresholds_sorted = [point[2] for point in static_points]
        
        plt.plot(static_recalls_sorted, static_precisions_sorted, 'b-', alpha=0.5, linewidth=3)
        plt.scatter(static_recalls_sorted, static_precisions_sorted, c='blue', marker='o', s=140, label='State of the Art')
        ...
        for i, threshold in enumerate(static_thresholds_sorted):
            if (i == 0 or i == len(static_thresholds_sorted) - 1):
                ...
                t = plt.text(
                    ...
                    static_precisions_sorted[i] + y_offset,  # Add offset to y-coordinate
                    f"{threshold}",
                    fontsize=20,
                    bbox=dict(boxstyle="round,pad=0.3", fc="aliceblue", ec="blue", alpha=0.8)
                )
                ...
                plt.annotate(
                    ...
                    xytext=(static_recalls_sorted[i], static_precisions_sorted[i] + y_offset),
    ...
    # Plot dynamic threshold points
    if dynamic_recalls:
        dynamic_points = sorted(zip(dynamic_recalls, dynamic_precisions, dynamic_rnd_ubs), key=lambda x: x[0])
        dynamic_recalls_sorted = [point[0] for point in dynamic_points]
        dynamic_precisions_sorted = [point[1] for point in dynamic_points]
        dynamic_rnd_ubs_sorted = [point[2] for point in dynamic_points]
        
        plt.plot(dynamic_recalls_sorted, dynamic_precisions_sorted, 'r-', alpha=0.7, linewidth=3)
        plt.scatter(dynamic_recalls_sorted, dynamic_precisions_sorted, c='red', marker='^', s=140, label='VectorQ')
        ...
        if vectorq_results:
            ...
            for i, rnd_ub in enumerate(dynamic_rnd_ubs_sorted):
                if rnd_ub in vectorq_results and len(vectorq_results[rnd_ub]['recalls']) > 1:
                    # Calculate 95% confidence interval
        ...
        for i, rnd_ub in enumerate(dynamic_rnd_ubs_sorted):
            if (i == 0 or i == len(dynamic_rnd_ubs_sorted) - 1):
                ...
                plt.annotate(
                    "",  # Empty text
                    xy=(dynamic_recalls_sorted[i], dynamic_precisions_sorted[i]),  # Start at the data point
                    xytext=(dynamic_recalls_sorted[i], dynamic_precisions_sorted[i] + y_offset),  # End at the label location
                    arrowprops=dict(arrowstyle="->", color="red", lw=1.2, alpha=0.7),  # Arrow style
...
def plot_duration_comparison(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    ...
    # Dictionary to group vectorq results by rnd_num_ub
    vectorq_results = {}  # {rnd_num_ub: {'durations': []}}
    ...
    # Process dynamic threshold files and group by rnd_num_ub
    for file_path in dynamic_files:
        with open(file_path, 'r') as f:
            data = json.load(f)
            ...
            vectorq_results[rnd_num_ub]['durations'].append(total_duration)
            
            if direct_duration is None:
                direct_duration = data["total_duration_direct_list"][-1] / 60
    
    # Calculate average durations for each rnd_num_ub
    dynamic_durations = []
    dynamic_rnd_ubs = []
    ...
    sorted_static = sorted(zip(static_thresholds, static_durations), key=lambda x: float(x[0]), reverse=True)
    ...
    plt.bar([bar_positions[0]], [all_durations[0]], width, color='gray', label='No Cache')
    
    static_indices = range(1, len(static_durations) + 1)
    plt.bar([bar_positions[i] for i in static_indices], 
        [all_durations[i] for i in static_indices], 
        width, color='blue', alpha=0.7, label='State of the Art')
    
    dynamic_indices = range(len(static_durations) + 1, len(all_durations))
    plt.bar([bar_positions[i] for i in dynamic_indices], 
        [all_durations[i] for i in dynamic_indices], 
        width, color='red', alpha=0.7, label='VectorQ')
    
    plt.xlabel('Threshold / rnd_num_ub', fontsize=FONT_SIZE)
...
def plot_duration_vs_error_rate(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    ...
    # Dictionary to group vectorq results by rnd_num_ub
    vectorq_results = {}  # {rnd_num_ub: {'durations': [], 'error_rates': []}}
    ...
    for file_path in dynamic_files:
        with open(file_path, 'r') as f:
            ...
            # Group results by rnd_num_ub for confidence intervals
            if "vectorq_" in file_path:
                if rnd_num_ub not in vectorq_results:
                    vectorq_results[rnd_num_ub] = {'durations': [], 'error_rates': [], 'samples': []}
                
                vectorq_results[rnd_num_ub]['durations'].append(total_duration)
                vectorq_results[rnd_num_ub]['error_rates'].append(error_rate)
                vectorq_results[rnd_num_ub]['samples'].append(num_samples)
    
    # For new runs with multiple files per rnd_num_ub, compute average
    if vectorq_results:
        for rnd_num_ub, values in sorted(vectorq_results.items(), key=lambda x: float(x[0])):
    ...
    if static_files:
        with open(static_files[0], 'r') as f:
            data = json.load(f)
            if "total_duration_direct_list" in data and data["total_duration_direct_list"]:
    ...
    if static_durations:
        static_points = sorted(zip(static_durations, static_error_rates, static_thresholds), key=lambda x: x[0])
        static_durations_sorted = [point[0] for point in static_points]
        static_error_rates_sorted = [point[1] for point in static_points]
        static_thresholds_sorted = [point[2] for point in static_points]

        plt.plot(static_durations_sorted, static_error_rates_sorted, 'b-', alpha=0.5, linewidth=3)
        plt.scatter(static_durations_sorted, static_error_rates_sorted, c='blue', marker='o', s=140, label='State of the Art')
    ...
    if dynamic_durations:
        dynamic_points = sorted(zip(dynamic_durations, dynamic_error_rates, dynamic_rnd_ubs), key=lambda x: x[0])
        dynamic_durations_sorted = [point[0] for point in dynamic_points]
        dynamic_error_rates_sorted = [point[1] for point in dynamic_points]
        dynamic_rnd_ubs_sorted = [point[2] for point in dynamic_points]

        plt.plot(dynamic_durations_sorted, dynamic_error_rates_sorted, 'r-', alpha=0.7, linewidth=3)
        ...
        plt.scatter(dynamic_durations_sorted, dynamic_error_rates_sorted, c='red', marker='^', s=140, label='VectorQ')
        ...
        if vectorq_results:
            ...
            for i, rnd_ub in enumerate(dynamic_rnd_ubs_sorted):
                if rnd_ub in vectorq_results and len(vectorq_results[rnd_ub]['durations']) > 1:
                    # Calculate 95% confidence interval
    ...
    if direct_duration is not None:
        ...
        plt.text(text_x, (ylim[1] - ylim[0]) * 0.9 + ylim[0], 
                f'No Cache: {direct_duration:.1f} min', 
                fontsize=FONT_SIZE-2, rotation=rotation, va=text_va, ha=text_ha, color='grey')
...
def plot_cache_hit_latency_vs_size_comparison(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    ...
    dynamic_data = {}  # {rnd_num_ub: {'cache_sizes': [], 'hit_latencies': []}}
    ...
    for file_path in dynamic_files:
        with open(file_path, 'r') as f:
            ...
            if len(cache_sizes) > 1:
                dynamic_data[rnd_num_ub] = {
                    'cache_sizes': cache_sizes,
                    'hit_latencies': hit_latencies
                }
    
    if not static_data and not dynamic_data:
        print(f"No cache hit data found for {dataset}, {embedding_model_name}, {llm_model_name}")
        return
    ...
    for i, (threshold, values) in enumerate(sorted(static_data.items())):
        plt.scatter(values['cache_sizes'], values['hit_latencies'], 
                   color=static_blues[i], marker='o', s=140, 
                   label=f'Static {threshold}', alpha=0.7)
    
    dynamic_reds = plt.cm.Reds(np.linspace(0.5, 1.0, len(dynamic_data)))
    for i, (rnd_num_ub, values) in enumerate(sorted(dynamic_data.items())):
        plt.scatter(values['cache_sizes'], values['hit_latencies'], 
                   color=dynamic_reds[i], marker='^', s=140, 
                   label=f'Dynamic {rnd_num_ub}', alpha=0.7)
    ...
    if len(all_static_sizes) > 5:
        try:
            ...
            plt.plot(x_range, p_static(x_range), 'b-', linewidth=3, 
    ...
    if len(all_dynamic_sizes) > 5:
        try:
            ...
            plt.plot(x_range, p_dynamic(x_range), 'r-', linewidth=3, 
    ...
    static_indices = [i for i, label in enumerate(labels) if 'Static' in label and 'Trend' not in label]
    dynamic_indices = [i for i, label in enumerate(labels) if 'Dynamic' in label and 'Trend' not in label]
    ...
    ordered_indices = [p[1] for p in static_pairs] + [p[1] for p in dynamic_pairs] + trend_indices
    plt.legend([handles[i] for i in ordered_indices], [labels[i] for i in ordered_indices],
              loc='best', ncol=2)
...
def plot_hit_rate_vs_latency(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    ...
    for file_path in dynamic_files:
        with open(file_path, 'r') as f:
            ...
            if hit_latencies:
                ...
                if "vectorq_" in file_path:
                    if rnd_num_ub not in vectorq_results:
                        vectorq_results[rnd_num_ub] = {'hit_rates': [], 'latencies': []}
                    
                    vectorq_results[rnd_num_ub]['hit_rates'].append(hit_rate)
                    vectorq_results[rnd_num_ub]['latencies'].append(avg_latency)
                else:
                    # Legacy support for old files
                    dynamic_hit_rates.append(hit_rate)
    ...
    if static_hit_rates:
        static_points = sorted(zip(static_hit_rates, static_latencies, static_thresholds), key=lambda x: x[0])
        static_hit_rates_sorted = [point[0] for point in static_points]
        static_latencies_sorted = [point[1] for point in static_points]
        static_thresholds_sorted = [point[2] for point in static_points]
        
        plt.plot(static_hit_rates_sorted, static_latencies_sorted, 'b-', alpha=0.5, linewidth=3)
        plt.scatter(static_hit_rates_sorted, static_latencies_sorted, c='blue', marker='o', 
                    s=140, label='State of the Art')

    if dynamic_hit_rates:
        dynamic_points = sorted(zip(dynamic_hit_rates, dynamic_latencies, dynamic_rnd_ubs), key=lambda x: x[0])
        dynamic_hit_rates_sorted = [point[0] for point in dynamic_points]
        dynamic_latencies_sorted = [point[1] for point in dynamic_points]
        dynamic_rnd_ubs_sorted = [point[2] for point in dynamic_points]
        
        plt.plot(dynamic_hit_rates_sorted, dynamic_latencies_sorted, 'r-', alpha=0.7, linewidth=3)
        plt.scatter(dynamic_hit_rates_sorted, dynamic_latencies_sorted, c='red', marker='^', 
                   s=140, label='VectorQ')
...
def plot_roc_curve(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE=20):
    static_files, dynamic_files = _get_result_files(dataset, embedding_model_name, llm_model_name, timestamp, results_dir)
    ...
    for file_path in static_files:
        with open(file_path, 'r') as f:
            ...
            if "true_positive_list" in data and "false_positive_list" in data and \
               "true_negative_list" in data and "false_negative_list" in data:
    ...
    if static_y_true and static_y_scores:
        ...
        plt.plot(fpr, tpr, 'b-', lw=3, alpha=0.7, 
    ...
    if dynamic_y_true and dynamic_y_scores:
        ...
        plt.plot(fpr, tpr, 'r-', lw=3, alpha=0.7,
                 label=f'VectorQ (AUC = {roc_auc:.3f})')
