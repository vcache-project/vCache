...
def __plot_accuracy_precision_recall_f1_score(benchmark: "Benchmark", font_size: int):
    ...
    f1_score_acc_list = compute_f1_score_acc_list(tp=benchmark.true_positive_acc_list, fp=benchmark.false_positive_acc_list, fn=benchmark.false_negative_acc_list)
    ...
    filename = (
        benchmark.output_folder_path + f"/accuracy_precision_recall_f1_score_{benchmark.timestamp}.pdf"
    )
...
def __plot_error_rate_cache_hit_rate_duration_avg_latency(benchmark: "Benchmark", font_size: int):
    ...
    duration_direct_acc_list = compute_duration_acc_list(latency_list=benchmark.latency_direct_list)
    ...
    axes[1, 0].plot(sample_index, duration_vectorq_acc_list, 'b-', linewidth=2)
    axes[1, 0].set_title('VectorQ Cumulative Duration', fontsize=font_size)
    axes[1, 0].set_xlabel('Samples', fontsize=font_size)
    axes[1, 0].set_ylabel('Duration (s)', fontsize=font_size)
    axes[1, 0].grid(True)
    axes[1, 0].tick_params(axis='both', labelsize=font_size-2)
    
    # Plot Direct Duration
    axes[1, 1].plot(sample_index, duration_direct_acc_list, 'm-', linewidth=2)
    axes[1, 1].set_title('Direct Cumulative Duration', fontsize=font_size)
    axes[1, 1].set_xlabel('Samples', fontsize=font_size)
    axes[1, 1].set_ylabel('Duration (s)', fontsize=font_size)
    axes[1, 1].grid(True)
    axes[1, 1].tick_params(axis='both', labelsize=font_size-2)
    ...
    filename = (
        benchmark.output_folder_path + f"/error_rate_cache_hit_rate_duration_avg_latency_{benchmark.timestamp}.pdf"
    )
    plt.savefig(filename, format="pdf")
    plt.close()
...
def __plot_avg_latency_cache_hit_rate_cache_miss_rate(benchmark: "Benchmark"):
    ...
    latency_vectorq_cache_hit_list = benchmark.latency_vectorq_list[benchmark.cache_hit_acc_list]
    latency_vectorq_cache_miss_list = benchmark.latency_vectorq_list[benchmark.cache_miss_acc_list]
    avg_latency_vectorq_cache_hit = compute_avg_latency_score(latency_list=latency_vectorq_cache_hit_list)
    avg_latency_vectorq_cache_miss = compute_avg_latency_score(latency_list=latency_vectorq_cache_miss_list)
    
    latency_direct_cache_hit_list = benchmark.latency_direct_list[benchmark.cache_hit_acc_list]
    latency_direct_cache_miss_list = benchmark.latency_direct_list[benchmark.cache_miss_acc_list]
    avg_latency_direct_cache_hit = compute_avg_latency_score(latency_list=latency_direct_cache_hit_list)
    avg_latency_direct_cache_miss = compute_avg_latency_score(latency_list=latency_direct_cache_miss_list)
    
    cache_hit_rate_vectorq = compute_cache_hit_rate_score(cache_hit_list_acc=benchmark.cache_hit_acc_list, cache_miss_list_acc=benchmark.cache_miss_acc_list)
    cache_miss_rate_vectorq = compute_cache_miss_rate_score(cache_miss_list_acc=benchmark.cache_miss_acc_list, cache_hit_list_acc=benchmark.cache_hit_acc_list)
    ...
    statistics = {
        "avg_latency": {
            ...
            "direct": {
                ...
                "cache_hit": float(avg_latency_direct_cache_hit),
                "cache_miss": float(avg_latency_direct_cache_miss)
            },
            "difference": {
                "overall": float(avg_latency_direct_overall - avg_latency_vectorq_overall),
                "cache_hit": float(avg_latency_direct_cache_hit - avg_latency_vectorq_cache_hit),
                "cache_miss": float(avg_latency_direct_cache_miss - avg_latency_vectorq_cache_miss)
            },
            "ratio": {
                "overall": float(avg_latency_direct_overall / avg_latency_vectorq_overall) if avg_latency_vectorq_overall > 0 else "N/A",
                "cache_hit": float(avg_latency_direct_cache_hit / avg_latency_vectorq_cache_hit) if avg_latency_vectorq_cache_hit > 0 else "N/A",
                "cache_miss": float(avg_latency_direct_cache_miss / avg_latency_vectorq_cache_miss) if avg_latency_vectorq_cache_miss > 0 else "N/A"
            }
        },
        "cache": {
            "hit_rate": float(cache_hit_rate_vectorq),
            "miss_rate": float(cache_miss_rate_vectorq),
            "total_samples": int(len(benchmark.latency_vectorq_list)),
            "hits": int(benchmark.cache_hit_acc_list.iloc[-1]),
            "misses": int(benchmark.cache_miss_acc_list.iloc[-1])
        }
    }
    
    filename = (
        benchmark.output_folder_path + f"/statistics_{benchmark.timestamp}.json"
    )
