...
########################################################################################################################
########################################################################################################################
def _generate_combination_plots(dataset, embedding_model_name, llm_model_name, timestamp, results_dir):
    plot_roc_curve(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    plot_hit_rate_vs_error(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    plot_precision_vs_recall(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    plot_duration_comparison(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    plot_duration_vs_error_rate(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    plot_cache_hit_latency_vs_size_comparison(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    plot_hit_rate_vs_latency(dataset, embedding_model_name, llm_model_name, timestamp, results_dir, FONT_SIZE)
    
def _generate_individual_plots(dataset, emb_model, llm_model, llm_model_path, results_dir, timestamp):

    static_dirs = [d for d in os.listdir(llm_model_path) 
                    if d.startswith('static_') and os.path.isdir(os.path.join(llm_model_path, d))]              
    vectorq_dirs = [d for d in os.listdir(llm_model_path) 
    ...
    for static_dir in static_dirs:
        ...
        result_files = [f for f in os.listdir(static_path) if f.startswith('results_') and f.endswith('.json')]
        ...
        for result_file in result_files:
            ...
            try:
                with open(json_path, 'r') as f:
                    data = json.load(f)
                benchmark_object = _create_benchmark_object(json_path, data, dataset, emb_model, llm_model, 
                                                          is_dynamic=False, results_dir=results_dir, timestamp=timestamp)
                _generate_plots_for_individual_benchmark(benchmark_object, is_dynamic=False)
    ...
    for vectorq_dir in vectorq_dirs:
        ...
        result_files = [f for f in os.listdir(vectorq_path) if f.startswith('results_') and f.endswith('.json')]
        ...
        for result_file in result_files:
            ...
            try:
                with open(json_path, 'r') as f:
                    data = json.load(f)
                benchmark_object = _create_benchmark_object(json_path, data, dataset, emb_model, llm_model, 
                                                          is_dynamic=True, results_dir=results_dir, timestamp=timestamp)
                _generate_plots_for_individual_benchmark(benchmark_object, is_dynamic=True)
            except Exception as e:
                print(f"        Error processing {json_path}: {str(e)}")

def _create_benchmark_object(json_path, data, dataset, emb_model, llm_model, is_dynamic, results_dir, timestamp):    
    ...
    benchmark_object.error_rates_relative_to_reused_answers = data["error_rates_relative_to_reused_answers"]
    benchmark_object.error_rates_relative_to_step_size = data.get("error_rates_relative_to_step_size")
    ...
    benchmark_object.inference_time_direct_step_size = data.get("inference_time_direct_step_size")
    benchmark_object.inference_time_vectorq_step_size = data.get("inference_time_vectorq_step_size")
...
def _generate_plots_for_individual_benchmark(benchmark_object: Benchmark, is_dynamic: bool):
...
DATASET_NAMES = ['semantic_prompt_cache_benchmark', 'ecommerce_dataset', 'amazon_instant_video']
    ...
    for dataset in datasets:
        ...
        for emb_model in embedding_models:
            ...
            if not os.path.exists(emb_model_path):
                print(f"  Embedding model path not found: {emb_model_path}, skipping...")
                continue
            ...
            for llm_model in llm_models:
                ...
                if not os.path.exists(llm_model_path):
                    print(f"    LLM model path not found: {llm_model_path}, skipping...")
                    continue
                print(f"    Processing LLM model: {llm_model}")
                
                timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M')
                if GENERATE_INDIVIDUAL_PLOTS:
                    _generate_individual_plots(dataset, emb_model, llm_model, llm_model_path, results_dir, timestamp)
                if GENERATE_COMBINATION_PLOTS:
                    _generate_combination_plots(dataset, emb_model, llm_model, timestamp, results_dir)
        print("\n")
