...
class VectorQCore:
    ...
    def process_request(
        self,
        prompt: str,
        benchmark: "VectorQBenchmark" = None,
    ...
    def __naive(
        ...
        if self.cache.is_empty():
            ...
            self.cache.add_embedding(
                embedding=benchmark.candidate_embedding, response=generated_response
            )
            return False, generated_response
        else:
            knn: List[tuple[float, int]] = self.cache.get_knn(
                prompt, k=1, embedding=benchmark.candidate_embedding
            )
            ...
            if sim >= self.vectorq_config.static_threshold:
                ...
                self.cache.add_embedding(
                    embedding=embedding, response=generated_response
                )
    ...
    def __vectorQ(
        ...
        if self.cache.is_empty():
            ...
            _, embedding_id = knn[0]
            self.cache.get_metadata(embedding_id=embedding_id)
