...
class VectorQCore():
    ...
    def process_request(self, prompt: str, benchmark: Optional["VectorQBenchmark"], output_format: Optional[str]) -> tuple[bool, str]:
        '''
    ...
    def __naive(self, prompt: str, benchmark: Optional["VectorQBenchmark"], output_format: Optional[str]) -> tuple[bool, str]:
    ...
    def __vectorQ(self, prompt: str, benchmark: Optional["VectorQBenchmark"], output_format: Optional[str]) -> tuple[bool, str]:
        if (self.cache.is_empty()):
            response:str = self.__create(prompt=prompt, benchmark=benchmark, output_format=output_format)
            self.__add(prompt=prompt, response=response, benchmark=benchmark)
            return False, response
    ...
    def __create(self, prompt:str, benchmark:Optional["VectorQBenchmark"], output_format:Optional[str]) -> str:
        if benchmark is not None:
            return benchmark.candidate_response
        else:
            return self.inference_engine.create(prompt=prompt, output_format=output_format)
        
    def __add(self, prompt:str, response:str, benchmark:Optional["VectorQBenchmark"]) -> int:
        if benchmark is not None:
            return self.cache.add_embedding(embedding=benchmark.candidate_embedding, response=response)
        else:
